local timespend, start_time = 0, 0
local RaidStart, RaidEnd, GetRaidList, CreateEvent, addDefaultOptions, ShowExportString
...
function CreateExportString(raid_id, event_id)
	if not raid_id or type(raid_id) ~= "number" then
		return "raid_id failed", 0, 0
	end
	if not event_id or type(event_id) ~= "number" then
